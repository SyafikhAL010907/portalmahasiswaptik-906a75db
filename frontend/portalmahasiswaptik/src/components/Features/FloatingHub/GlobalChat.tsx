...
import { UserProfileModal } from '@/components/Features/Dashboard/UserProfileModal';
...
export function GlobalChat() {
    ...
    // Custom event listener for external triggers (e.g., BottomNav)
    useEffect(() => {
    ...
    return (
        <>
            {!isMobileView && (
                <div className="hidden md:block fixed bottom-32 md:bottom-6 right-6 z-[100]">
                    <Button
                        onClick={() => {
                            if (isOpen) {
                                setIsOpen(false);
            ...
            {isOpen && (
                <div className={cn(
                    "fixed bottom-40 md:bottom-24 right-6 w-[350px] sm:w-[400px] h-[600px] max-h-[80vh] z-[100] flex flex-col overflow-hidden animate-in slide-in-from-bottom-5 duration-300",
