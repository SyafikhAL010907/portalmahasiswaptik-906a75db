...
export function PaymentNotificationCenter() {
    ...
    useEffect(() => {
        ...
        const channel = supabase
            .channel(`admin_payment_updates_${Math.random().toString(36).substring(7)}`) // Unique channel to avoid collisions
            .on(
                'postgres_changes',
                { event: '*', schema: 'public', table: 'weekly_dues' },
                (payload) => {
                    console.log("🔄 Admin Queue Real-time:", payload.eventType, payload.new);
                    fetchData(); // Aggressive refresh on any change
                }
            )
            .subscribe((status) => {
                console.log("📡 Admin Queue Subscription Status:", status);
            });
    ...
    // 🚀 Custom Event Listener for instant local sync (Triggered by usePayment)
    useEffect(() => {
        const handleLocalRefresh = () => {
            console.log("🚀 Admin Queue: Local Manual Refresh Triggered!");
            fetchData();
        };
        window.addEventListener('refresh-admin-queue', handleLocalRefresh);
        return () => window.removeEventListener('refresh-admin-queue', handleLocalRefresh);
    }, [hasAccess]);
