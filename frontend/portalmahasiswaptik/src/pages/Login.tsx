...
import { Eye, EyeOff, GraduationCap, ArrowLeft, MessageCircle, Loader2, Fingerprint } from 'lucide-react';
...
export default function Login() {
  ...
  const handleBiometricLogin = async () => {
    if (!nim) {
      toast.error('Masukkan NIM/NIP lo dulu bro biar sistem tau siapa yang mau login!');
      return;
    }

    setIsLoading(true);
    try {
      const result = await webauthnService.authenticate(nim);
      if (result.success && result.token) {
        // Log in to Supabase using the custom JWT generated by our Go backend
        const { error } = await supabase.auth.setSession({
          access_token: result.token,
          refresh_token: '', // Custom tokens don't have refresh tokens
        });

        if (error) throw error;

        toast.success('Berhasil Masuk via Biometrik! 🦾');
        navigate('/dashboard');
      }
    } catch (err: any) {
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <button
                ...
                className="absolute right-3 top-1/2 -translate-y-1/2 w-8 h-8 rounded-lg bg-primary/10 text-primary hover:bg-primary hover:text-white transition-all flex items-center justify-center border border-primary/20"
                title="Masuk pake Biometrik"
              >
