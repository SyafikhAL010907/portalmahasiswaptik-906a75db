            ...
            <div className="space-y-3 mb-6">
              ...
              {unpaidWeeks.length > 0 ? (
                unpaidWeeks.map((item) => (
                  <div key={item.week} className="flex justify-between items-start p-4 bg-muted/30 rounded-xl border border-border/50">
                    <div className="flex flex-col gap-1">
                      <span className="text-sm font-medium">Iuran Minggu ke-{item.week}</span>
                      {item.status === 'Pending' && (
                        <span className="text-[10px] text-amber-600 font-bold bg-amber-100 px-2 py-0.5 rounded-full w-fit italic whitespace-normal text-left h-auto leading-tight">Menunggu Verifikasi Bendahara</span>
                      )}
                    </div>
                    <span className="text-sm font-bold text-destructive/80 self-center">Rp 5.000</span>
                  </div>
                ))
              ) : (
            ...
            {totalBill > 0 && (
              <div className="pt-6 border-t border-border space-y-4">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                  ...
                  <Button onClick={() => setShowQRIS(true)} className="bg-primary hover:scale-105 transition-transform rounded-xl px-6 py-2 h-12 w-full sm:w-auto">
                    Bayar Sekarang
                  </Button>
                </div>
              </div>
