...
export default function Competitions() {
    ...
    return (
        ...
        >
            ...
            {/* Loading / Cards */}
            {loading ? (
                <motion.div variants={staggerBottom as any} layout={false} className="grid grid-cols-1 lg:grid-cols-2 2xl:grid-cols-3 gap-4 lg:gap-6">
                    {[1, 2, 3, 4, 5, 6].map(i => (
                        <div key={i} className="h-64 rounded-2xl bg-secondary/20 animate-pulse" />
                    ))}
                </motion.div>
            ) : (
                <motion.div variants={staggerBottom as any} layout={false} className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 lg:gap-6">
