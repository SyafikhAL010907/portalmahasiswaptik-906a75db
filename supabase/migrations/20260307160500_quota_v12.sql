-- =============================================
-- Migration: Quota Logic V12 (Final Enforcement)
-- Purpose: Add missing columns & strict per-file quota
-- =============================================

-- 1. Add Missing Columns to download_logs
DO $$ 
BEGIN 
...
-- Required because return type/OUT parameters changed
...
BEGIN
    ...
    ELSIF _role IN ('admin_kelas', 'admin kelas') THEN
        -- Admin Kelas Rules (Global per type / 24 hours)
        CASE _type
            WHEN 'finance' THEN
                quota_limit := 2;
                window_interval := INTERVAL '7 days';
                is_per_resource := FALSE; -- Shared across files
...
-- 3. Ensure Permissions (Strict: Disable RLS for backend reliability)
